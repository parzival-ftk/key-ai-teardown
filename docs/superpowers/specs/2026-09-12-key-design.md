...
- 用途是**面试**——把它放进项目集，帮助通过面试（目标岗位：**AI 产品经理 / 产品经理**）。
...
1. 不做多用户账号体系 / 权限 / 团队协作（面试演示不需要，YAGNI）。
2. 不做商业化（支付、计费、多租户）。
...
目标用户：产品经理、创业者；面试演示场景下，**面试官**是首要"观众"。
...
| 层 | 选型 | 理由 |
|---|---|---|
| 框架 | **Next.js 15（App Router）+ TypeScript** | 前后端一体、SSE 流式天然支持、Vercel 一键部署，面试官点开链接即可体验 |
...
面试高光画面：左侧 Agent 流水线逐个点亮，右侧报告实时生长。
...
| **Wave 6** | 演示打磨（动效、空状态、错误兜底、样例数据） | 面试就绪 |
...
| 面试时网络/额度异常 | 演示翻车 | 预置"样例数据回放"演示模式，断网也能演示完整流程 |
