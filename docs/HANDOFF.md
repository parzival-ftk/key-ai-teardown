...
**这个仓库在做什么**：**Key —— AI-powered interface analysis and visual decomposition workspace**。上传产品界面截图，拆解成结构化组件树，在无限画布上组织与检查，并通过 ComfyUI 生成视觉资产。（历史：仓库早期包含一套「产品拆解」分析能力——竞品 / 用户与 JTBD / 访谈 / 视觉设计 / 商业模式 / 反方质疑 / 答辩 / 综合裁决 / PRD / 界面代码，以及多产品并列对比矩阵，作为 legacy 模块保留。）
