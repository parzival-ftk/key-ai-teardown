 ...
 *   → 真实全编队（10 个 Agent，真实框架提示词）
 *   → 真实编排（并行 5 + 串行 5：访谈官显式依赖研究员；质疑→答辩→综合串行链）
 *   → 真实 SSE 序列化
...
describe("端到端：真实 HTTP + 全编队（唯一替身是 stub LLM server）", () => {
  ...
  it("全编队经真实 HTTP 走完：10 个 Agent 逐个产出、SSE 格式正确、以 done 收尾、无 error", async () => {
    ...
    expect(doneEvents).toHaveLength(10);
